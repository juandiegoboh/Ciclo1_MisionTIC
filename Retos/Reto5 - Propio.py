# -*- coding: utf-8 -*-
"""
Created on Sun Jun 13 11:24:10 2021
...
{
     "precio promedio maximo": <precio_promedio_maximo>,
     "precio promedio minimo": <precio_promedio_minimo>,
     "volumen maximo": volumen maximo,
     "fecha maximo promedio": <fecha>,
     "fecha minimo promedio": <fecha>,
     "tipo con maximo precio": <tipo>,
}

Funciones que se deben crear:
    Filtrar solo por la llave de lugar, se usan los indices en donde aparece el lugar
    
    Funcion para obtener listas de fechas, tipos, precios, volumenes. En el lugar

        Funcion para obtener el precio maximo
        Funcion para obtener el precio minimo
        Funcion para obtener la fecha en ese precio maximo
        Funcion para obtener la fecha en ese precio minimo
        Funcion para obtener el tipo en ese precio maximo
    
    Una vez obtenidos todas las entradas se crea una funcion que cree el diccionario resultante
"""
...
def get_min_price(prices_list: list):
    min_price = min(prices_list)
    
    return min_price

def get_max_price(prices_list: list):
    max_price = max(prices_list)
    
    return max_price

def get_max_volume(volumes_list: list):
    max_volume = max(volumes_list)
    
    return max_volume

def get_max_date(dates_list: list, prices_list: list):
...
def get_min_date(dates_list: list, prices_list: list):
    min_price = get_min_price(prices_list)
...
def get_type(types_list, prices_list):
    max_price = max(prices_list)
    
    for tipo, price in zip(types_list, prices_list):
        if price == max_price:
            max_tipo = tipo
    
    return max_tipo

def construct_dict(dates_list: list, prices_list: list, types_list: list, volumes_list: list):
    
    # Genero las variables que iran en el diccionario, tomo en cuenta las listas, principalmente las de precio
    precio_promedio_maximo = get_max_price(prices_list)
    precio_promedio_minimo = get_min_price(prices_list)
    volumen_maximo = get_max_volume(volumes_list)  
    fecha_maximo = get_max_date(dates_list, prices_list)    # De acá para abajo se usa la lista de precios también
    fecha_minimo = get_min_date(dates_list, prices_list)  
    tipo = get_type(types_list, prices_list)
    
    diccionario = {
     "precio promedio maximo": precio_promedio_maximo,
     "precio promedio minimo": precio_promedio_minimo,
     "volumen maximo": volumen_maximo,
     "fecha maximo promedio": fecha_maximo,
     "fecha minimo promedio": fecha_minimo,
     "tipo con maximo precio": tipo,
}
    
    return diccionario

def get_data(data: dict, region: str):

    # Dictionaries
    dict_region = data["region"]
    dict_dates = data["Date"]
    dict_prices = data["AveragePrice"]
    dict_types = data["type"]   
    dict_volumes = data["Total Volume"]
    ...
    for index, value in dict_region.items():
        if value == region:
            dates_list.append(dict_dates[index])
            prices_list.append(dict_prices[index])
            types_list.append(dict_types[index])
            volumes_list.append(dict_volumes[index])
    
    return dates_list, prices_list, types_list, volumes_list

def get_general_statics(data: dict, region: str):
    if region not in list(data["region"].values()):
        return None
    else:
        dates_list, prices_list, types_list, volumes_list = get_data(data, region)

    result = construct_dict(dates_list, prices_list, types_list, volumes_list)
